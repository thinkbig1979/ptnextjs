...
cp /home/edwin/development/ptnextjs/lib/types.ts /home/edwin/development/ptnextjs/lib/types.ts.backup

# Create the updated Product interface section
cat > /tmp/product-updated.txt << 'PRODUCT_SECTION'
...
# Extract everything before Product interface (lines 1-795)
head -n 795 /home/edwin/development/ptnextjs/lib/types.ts > /tmp/types-part1.txt

# Extract everything after Product interface (lines 855+)
tail -n +855 /home/edwin/development/ptnextjs/lib/types.ts > /tmp/types-part3.txt

# Concatenate: before + updated Product + API responses + after
cat /tmp/types-part1.txt /tmp/product-updated.txt /tmp/types-part3.txt > /home/edwin/development/ptnextjs/lib/types.ts

echo "Successfully updated lib/types.ts"
echo "- Updated Product interface with Payload CMS support"
echo "- Added Product API response types"
echo "- Added generic ApiErrorResponse type"
echo ""
echo "Backup saved to: lib/types.ts.backup"
