...
import { Target, TrendingUp, Eye, CheckCircle, ArrowRight } from "lucide-react";

export const metadata: Metadata = {
  title: "Vendor Consultancy | Paul Thames",
  description: "Market access and visibility for manufacturers, distributors, and technology providers in the superyacht industry. Proposition testing, market strategy, and directory listings.",
};
        ...
        <div className="mb-20">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {services.map((service) => (
              <Card key={service.title} className="flex flex-col h-full border-accent/10 hover:border-accent/30 transition-colors">
                <CardHeader>
                  <div className="w-14 h-14 bg-accent/10 border border-accent/20 rounded-xl flex items-center justify-center mb-4">
                    <service.icon className="w-7 h-7 text-accent" />
                  </div>
                  <CardTitle className="text-xl font-cormorant text-accent">{service.title}</CardTitle>
                ...
                <CardContent className="flex-1 space-y-6">
                  {/* Includes */}
                  <div>
                    <h4 className="font-poppins-medium text-sm text-foreground mb-3">
                      Includes
                    </h4>
                    <ul className="space-y-2">
                      {service.includes.map((item) => (
                        <li key={item} className="flex items-start text-muted-foreground font-poppins-light text-sm">
                  ...
                  {/* For Whom */}
                  <div>
                    <h4 className="font-poppins-medium text-sm text-foreground mb-2">
                      For
                    </h4>
                  ...
                  <div className="bg-accent/5 rounded-lg p-4 border border-accent/10">
                    <h4 className="font-poppins-medium text-sm text-accent mb-2">
                      Outcome
                    </h4>
        ...
        <div className="mb-20">
          <div className="bg-secondary/30 rounded-2xl p-8 border border-border text-center">
            <p className="text-xl text-muted-foreground font-poppins-light max-w-3xl mx-auto">
              Industry insights and relationships without full-time overhead.
              Candid feedback on what works in this market and what doesn&apos;t.
            </p>
          </div>
        </div>
