...
test.describe('Location Search - Nantes France Debug', () => {
  test('should debug Nantes search issue', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="location-results-dropdown"]', { timeout: 10000 });
    ...
    // Get results text
    const resultsText = await page.locator('text=/Showing.*vendors/').first().textContent();
    console.log(`\n=== RESULTS: ${resultsText} ===\n`);
    ...
    const newResultsText = await page.locator('text=/Showing.*vendors/').first().textContent();
